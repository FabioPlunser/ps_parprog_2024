...
| FP_COMP_OPS_EXE.SSE_FP_PACKED | 0 | 1,022,338,138 | Counts the number of packed (multiple data in single instruction) single-precision floating-point SSE instructions. | **Very relevant** as it shows that auto-vectorization combines multiple data into single instructions. This is not the case on the non-vectorized version. |
